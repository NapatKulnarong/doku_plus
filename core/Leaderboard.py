...
class Leaderboard:
    def __init__(self, screen, user_manager):
        ...
        self.font = pygame.font.Font("assets/fonts/nunito_bold.ttf", int(20 * 1.3))  # 26
        self.title_font = pygame.font.Font("assets/fonts/nunito_bold_italic.ttf", int(54 * 1.3))  # 70
        self.header_font = pygame.font.Font("assets/fonts/nunito_bold.ttf", int(22 * 1.3))  # 29
        self.back_button = pygame.Rect(WIDTH // 2 - int(100 * 1.3), HEIGHT - int(70 * 1.3), int(200 * 1.3), int(45 * 1.3))  # -130, -91, 260, 59
    ...
    def draw(self):
        ...
        self.screen.blit(title, title.get_rect(center=(WIDTH // 2, int(50 * 1.3))))  # 65

        # Table settings
        headers = ["Rank", "Username", "Level"]
        col_widths = [int(80 * 1.3), int(280 * 1.3), int(100 * 1.3)]  # 104, 364, 130
        row_height = int(40 * 1.3)  # 52
        header_height = int(50 * 1.3)  # 65
        padding = int(10 * 1.3)  # 13

        table_width = sum(col_widths)
        table_height = header_height + self.visible_rows * row_height
        start_x = WIDTH // 2 - table_width // 2
        start_y = int(100 * 1.3)  # 130

        # Draw table background
        table_rect = pygame.Rect(start_x - padding, start_y - padding, table_width + 2 * padding, table_height + 2 * padding)
        ...
        for i, (name, stats) in enumerate(visible_players):
            ...
            for col, val in enumerate(values):
                ...
                rect = text.get_rect(center=(start_x + sum(col_widths[:col]) + col_widths[col] // 2, row_y + row_height // 2))
                self.screen.blit(text, rect)

        # Draw back button (already scaled in __init__)
        pygame.draw.rect(self.screen, GRAY, self.back_button, border_radius=int(5 * 1.3))  # 7
        pygame.draw.rect(self.screen, BLACK, self.back_button, 2, border_radius=int(5 * 1.3))  # 7
